#ifndef MOLBIOLIB_READONLYSEQUENCESFILE_H
#define MOLBIOLIB_READONLYSEQUENCESFILE_H 1
...
/** \file ReadOnlySequencesFile.hpp
 * Contains only the #ReadOnlySequencesFile class.
 */

/** Object that read-only fasta or fastq files with a vector interface.
 ...
 * \code
 * string FILE = ...;
 * ReadOnlySequencesFile myReads(FILE);
 ...
 * \code
 * ReadOnlySequencesFile myReads(FILE, true);
...
public:
    ...
    void getRead(size_t readNum)
    {
        ...
        if (readNum != currentRead)
        {
            ...
            else
            {
                ...
                fastaIndex.getRow(readNum, tempRow);
                currentHeader = ifp[get<0>(tempRow)].substr(1);
                size_t start = get<1>(tempRow),
                       end = get<1>(tempRow) + get<2>(tempRow);
                currentSeq = "";
                for (size_t i = start; i < end; ++i)
    ...
    size_t qualSize(size_t contig)
    {
        ...
        {
#ifdef PROG_DEBUG
            cerr << "Warning from ReadOnlySequencesFile.  qualSize called on a non-fastq file.  Using sequence length and default quality score." << endl;
#endif
            return currentSeq.size();
        }
    }
    ...
    string& getQualHeader(size_t readNum)
    {
        ...
        {
#ifdef PROG_DEBUG
            cerr << "Warning from ReadOnlySequencesFile.  getQualHeader called on a non-fastq file.  Using sequence header." << endl;
#endif
            return currentHeader;
        }
    }
    ...
    string& getQual(size_t readNum)
    {
        ...
        {
#ifdef PROG_DEBUG
            cerr << "Warning from ReadOnlySequencesFile.  getQual called on a non-fastq file.  Using sequence length and default quality score." << endl;
#endif
            size_t currSeqSize = currentSeq.size();
            // Below is to stop from recomputing the same string.
            if (currentQual.size() != currSeqSize)
            {
                currentQual.resize(currSeqSize);
                for (size_t i = 0; i < currSeqSize; ++i)
                {
                    currentQual[i] = qualDefaultChar;
                }
            }
        }
        return currentQual;
    ...
    char getQualChar(size_t contig, size_t pos)
    {
        ...
        {
#ifdef PROG_DEBUG
            cerr << "Warning from ReadOnlySequencesFile.  getQualChar called on a non-fastq file.  Using default quality score." << endl;
#endif
            return qualDefaultChar;
        }
    }
    ...
    int getQualScore(size_t contig, size_t pos, int offset = 0)
    {
        ...
        {
#ifdef PROG_DEBUG
            cerr << "Warning from ReadOnlySequencesFile.  getQualScore called on a non-fastq file.  Using default quality score." << endl;
#endif
            return qualDefault;
        }
    }
...
protected:
    size_t numReads;
    bool generateHeaderMap, sequentialMode, fastqMode;
